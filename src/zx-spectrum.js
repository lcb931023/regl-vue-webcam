...
void main(  )
{
    // vec2 pv = floor(  vec2(gl_FragCoord.x, (resolution.y-gl_FragCoord.y))  /LOWREZ); // this works too
    vec2 pv = floor(uv * resolution.xy / LOWREZ);
    vec2 bv = floor(pv/8.0)*8.0;
    vec2 sv = floor(resolution.xy/LOWREZ);
    ...
    for(int py=1;py<8;py++)
    {
        ...
        {
		    vec4 cs=bmap( (texture2D(webcam,(bv+vec2(px,py))/sv).rgb) );
            bright+=cs.a;
    ...
    vec3 cs=texture2D(webcam,pv/sv).rgb;
    vec3 d= (cs+cs) - (c1+c2) ;
